...
Q_LOGGING_CATEGORY(QT_PLATFORM_EMSCRIPTEN_CURSOR, "qt.platform.emscripten.cursor")
...
QEmscriptenCursor::QEmscriptenCursor()
    : QPlatformCursor()
{
}
...
void QEmscriptenCursor::changeCursor(QCursor *cursor, QWindow *window)
{
    const int shape = cursor ? cursor->shape() : Qt::CursorShape(Qt::ArrowCursor);
    qCDebug(QT_PLATFORM_EMSCRIPTEN_CURSOR) << "changeCursor" << shape << window;
    if (shape > Qt::LastCursor)
        qCWarning(QT_PLATFORM_EMSCRIPTEN_CURSOR) << "Custom cursors are not supported";
//    pp::MouseCursor::SetCursor(QEmscriptenInstancePrivate::getPPInstance(), cursorTypeForShape(shape),
